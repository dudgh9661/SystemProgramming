...
void* button(void *data) { // button_app
	...
	while (1) {
		...
		//		if (strlen(receive_button) == 7)	printf("Button Pressed_thread\n");
		...
		while (delay_touch != 5) {
			while (strlen(receive_touch) == 3) { // state: touch sensed. ( person sit )
				if (strlen(receive_button) == 7) { // when click the button, motors operate
					...
					//buzzer(3); 
					button_flag = 1; // button is pressed
					count = 0;
					continue; // 아래 실행 안되고 while 실행
				}
				// ultrasonic2(for counting poop sensor) operates
			...
			printf("%dth touch\n", delay_touch);
			if (button_flag == 1) { // if input the button signal
				button_flag = 0; // initailize button_flag
				break;
			}
